...
public abstract class DisruptorManager {
    //disruptor缓存
    private Map<String,Disruptor<OrderEvent>> disruptorCache=new ConcurrentHashMap<>();
    //event缓存
    private Map<String, OrderEventHandler> eventCache= new ConcurrentHashMap<>();
    //disruptorTemplate缓存
    private Map<String,DisruptorTemplate> disruptorTemplates=new ConcurrentHashMap<>();
     ...
     * @return
     */
    public RingBuffer<OrderEvent> initRingBuffer(String symbol,int coinScale,int baseCoinScale){
        //1.event事件工厂
...
//        BlockingWaitStrategy waitStrategy= new BlockingWaitStrategy(); //利用锁和等待机制的WaitStrategyCPU消耗少但是延迟比较高
        ...
        //5-2.获取order book
        OrderBook orderBooks=buildOrderBook(symbol,coinScale,baseCoinScale);
        //5-3.撮合事件
        ...
        disruptor.start();
        //6.对象添加到缓存
        disruptorCache.put(symbol,disruptor);
        eventCache.put(symbol,orderEventHandler);
        return ringBuffer;
    }
    ...
    public DisruptorTemplate build(String symbol, int coinScale, int baseCoinScale){
        RingBuffer<OrderEvent> orderEventRingBuffer = this.initRingBuffer(symbol,coinScale,baseCoinScale);
        DisruptorTemplate disruptorTemplate = new DisruptorTemplate(orderEventRingBuffer);
        disruptorTemplates.put(symbol,disruptorTemplate);
        return disruptorTemplate;
    }

    public void destroy(String symbol){
        Disruptor<OrderEvent> disruptor = disruptorCache.remove(symbol);
        if(disruptor!=null){
            disruptor.shutdown();
        }
        eventCache.remove(symbol);
    }
